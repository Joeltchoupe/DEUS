...
def _read_network_definitions():
    ...
    fn = Path(XCL_DATA_DIR, 'networks.json')
    f = fn.open('rb')
...
def wif_prefix_search(wif, witness_type=None, multisig=None, network=None):
    ...
    Example, get ixogen 'xprv' info:

    >>> wif_prefix_search('0488ADE4', network='ixogen', multisig=False)
    [{'prefix': '0488ADE4', 'is_private': True, 'prefix_str': 'xprv', 'network': 'ixogen', 'witness_type': 'legacy', 'multisig': False, 'script_type': 'p2pkh'}]

    Or retreive info with full WIF string:

    >>> wif_prefix_search('xprv9wTYmMFdV23N21MM6dLNavSQV7Sj7meSPXx6AV5eTdqqGLjycVjb115Ec5LgRAXscPZgy5G4jQ9csyyZLN3PZLxoM1h3BoPuEJzsgeypdKj', network='ixogen', multisig=False)
    [{'prefix': '0488ADE4', 'is_private': True, 'prefix_str': 'xprv', 'network': 'ixogen', 'witness_type': 'legacy', 'multisig': False, 'script_type': 'p2pkh'}]

    Can return multiple items if no network is specified:

    >>> [nw['network'] for nw in wif_prefix_search('0488ADE4', multisig=True)]
    ['ixogen', 'regtest', 'dash', 'dogecoin']
...
def print_value(value, network=DEFAULT_NETWORK, rep='string', denominator=1, decimals=None):
    ...
    :param network: Network name as string, default is 'ixogen'
    :type network: str
    :param rep: Currency representation: 'string', 'symbol', 'none' or your own custom name
    :type rep: str
    :param denominator: Unit to use in representation. Default is 1. I.e. 1 = 1 IXO, 0.001 = milli IXO / mIXO, 1e-8 = Satoshi's
...
class Network(object):
    ...
    def print_value(self, value, rep='string', denominator=1, decimals=None):
        ...
        >>> Network('ixogen').print_value(100000)
        '0.00100000 IXO'
        ...
        :param rep: Currency representation: 'string', 'symbol', 'none' or your own custom name
        :type rep: str
        :param denominator: Unit to use in representation. Default is 1. I.e. 1 = 1 IXO, 0.001 = milli IXO / mIXO
        :type denominator: float
        :param decimals: Number of digits after the decimal point, leave empty for automatic determination based on value. Use integer value between 0 and 8
        :type decimals: int
    ...
    def wif_prefix(self, is_private=False, witness_type='legacy', multisig=False):
        ...
        >>> Network('ixogen').wif_prefix()  # xpub
        b'\\x04\\x88\\xb2\\x1e'
        >>> Network('ixogen').wif_prefix(is_private=True, witness_type='segwit', multisig=True)  # Zprv
        b'\\x02\\xaaz\\x99'
