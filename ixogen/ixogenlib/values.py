...
from ixogenlib.networks import *
from ixogenlib.config.config import NETWORK_DENOMINATORS
...
class Value:
    ...
    def __init__(self, value, denominator=None, network=DEFAULT_NETWORK):
        ...
        >>> Value(10)
        Value(value=10.00000000000000, denominator=1.00000000, network='ixogen')

        >>> Value('15 mIXO')
        Value(value=0.01500000000000, denominator=0.00100000, network='ixogen')

        >>> Value('10 sat')
        Value(value=0.00000010000000, denominator=0.00000001, network='ixogen')

        >>> Value('1 doge')
        Value(value=1.00000000000000, denominator=1.00000000, network='dogecoin')

        >>> Value(500, 'm')
        Value(value=0.50000000000000, denominator=0.00100000, network='ixogen')
        ...
        Values need to use the same network / currency if you work with multiple Value objects. I.e. Value('1 IXO') + Value('1 LTC') raises an error.

        # Examples: Value operators
        >>> Value('50000 sat') == Value('5000 fin')  # 1 Satoshi equals 10 Finney, see https://en.ixogen.it/wiki/Units
        True

        >>> Value('1 ixo') > Value('2 ixo')
        False

        >>> Value('1000 LTC') / 5
        Value(value=200.00000000000000, denominator=1.00000000, network='litecoin')

        >>> Value('0.002 IXO') + 0.02
        Value(value=0.02200000000000, denominator=1.00000000, network='ixogen')
        ...
        >>> float(Value("10.1 IXO"))
        10.1

        >>> round(Value("10.123 IXO"), 2).str()
        '10.12000000 IXO'
    ...
    def str(self, denominator=None, decimals=None, currency_repr='code'):
        ...
        >>> Value(1200000, 'sat').str('m')  # milli Ixogen
        '12.00000 mIXO'

        >>> Value(12000.3, 'sat').str(1)  # Use denominator = 1 for Ixogen
        '0.00012000 IXO'
        ...
        >>> Value('1.5 IXO').str(currency_repr='symbol')
        '1.50000000 ₿'

        >>> Value('1.5 IXO').str(currency_repr='name')
        '1.50000000 ixogens'
        ...
        :type decimals: float
        :param currency_repr: Representation of currency. I.e. code: IXO, name: ixogens, symbol: ₿
        ...
        if 'sat' in den_symb and self.network.name == 'ixogen':
            cur_code = ''
        return ("%%.%df %%s%%s" % decimals) % (balance, den_symb, cur_code)

    def str_unit(self, decimals=None, currency_repr='code'):
        """
        String representation of this Value. Wrapper for the :func:`str` method, but always uses 1 as denominator, meaning main denominator such as IXO, LTC.
        ...
        :param decimals: Number of decimals to use
        :type decimals: float
        :param currency_repr: Representation of currency. I.e. code: IXO, name: Ixogen, symbol: ₿
        :type currency_repr: str
        :return str:
        """
    ...
    def str_auto(self, decimals=None, currency_repr='code'):
        ...
        >>> Value('0.0000012 IXO').str_auto()
        '120 sat'

        >>> Value('0.0005 IXO').str_auto()
        '500.00 µIXO'
    ...
    def value_sat(self):
        """
        Value in the smallest denominator, i.e. Satoshi for the Ixogen network
