...
#    Create and manage IxogenLib legacy/segwit single and multisignatures wallets from the commandline
#
...
from ixogenlib.wallets import Wallet, wallets_list, wallet_exists, wallet_delete, WalletError, wallet_empty
from ixogenlib.mnemonic import Mnemonic
from ixogenlib.keys import HDKey
from ixogenlib.main import IXOGENLIB_VERSION
...
DEFAULT_NETWORK = 'ixogen'


def parse_args():
    parser = argparse.ArgumentParser(description='IxogenLib command line wallet')
    ...
    group_wallet.add_argument('--list-wallets', '-l', action='store_true',
                              help="List all known wallets in IxogenLib database")
    ...
    group_wallet2.add_argument('--network', '-n',
                               help="Specify 'ixogen', 'litecoin', 'testnet' or other supported network")
...
def create_wallet(wallet_name, args, db_uri):
    ...
    if args.create_multisig:
        ...
        from ixogenlib.keys import get_key_format
...
def main():
    print("Command Line Wallet - IxogenLib %s\n" % IXOGENLIB_VERSION)
    ...
    if args.network == 'ixogenlib_test':
        wlt.utxos_update()
