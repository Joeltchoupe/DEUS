...
from ixogenlib.wallets import wallet_exists, Wallet
...
# from ixogenlib.wallets import wallet_delete_if_exists
...
if not wallet_exists(WALLET_NAME):
    ...
    print("from ixogenlib.wallets import Wallet")
    print("from ixogenlib.keys import HDKey")
    print("")
    print("key_list = [")
    ...
    from ixogenlib.config.config import IXOGENLIB_VERSION, XCL_DATABASE_DIR
    online_wallet = Wallet(WALLET_NAME, db_uri=XCL_DATABASE_DIR + '/ixogenlib.tmp.sqlite')
    online_wallet.utxos_update()
    online_wallet.info()
    utxos = online_wallet.utxos()
    if utxos:
        print("\nNew unspent outputs found!")
        print("Now a new transaction will be created to sweep this wallet and send ixogens to a testnet faucet")
        ...
        print("\nfrom ixogenlib.wallets import Wallet")
        print("")
