# -*- coding: utf-8 -*-
#    IxogenLib - Python Cryptocurrency Library
...
import numpy as np
import pickle
from ixogenlib.db import *
from ixogenlib.encoding import *
from ixogenlib.keys import Address, BKeyError, HDKey, check_network_and_key, path_expand
from ixogenlib.mnemonic import Mnemonic
from ixogenlib.networks import Network
from ixogenlib.values import Value, value_to_satoshi
from ixogenlib.services.services import Service
from ixogenlib.transactions import Input, Output, Transaction, get_unlocking_script_type
from ixogenlib.scripts import Script
...
class WalletTransaction(Transaction):
    ...
    def save(self, filename=None):
        """
        Store transaction object as file, so it can be imported in ixogenlib later with the :func:`load` method.

        :param filename: Location and name of file, leave empty to store transaction in ixogenlib data directory: .ixogenlib/<transaction_id.tx)
        :type filename: str

        :return:
        """
        if not filename:
            p = Path(XCL_DATA_DIR, '%s.tx' % self.txid)
        else:
            p = Path(filename)
            if not p.parent or str(p.parent) == '.':
                p = Path(XCL_DATA_DIR, filename)
        f = p.open('wb')
...
class Wallet(object):
    ...
    For the Ixogen network, Litecoin or any other network you define in the settings.
    ...
    def create(cls, name, keys=None, owner='', network=None, account_id=0, purpose=0, scheme='bip32',
        ...
        >>> if wallet_delete_if_exists('ixogenlib_legacy_wallet_test', force=True): pass
        >>> w = Wallet.create('ixogenlib_legacy_wallet_test', wif)
        >>> w
        <Wallet(name="ixogenlib_legacy_wallet_test")>
    ...
    def keys(self, account_id=None, name=None, key_id=None, change=None, depth=None, used=None, is_private=None,
        ...
        >>> w = Wallet('ixogenlib_legacy_wallet_test')
        >>> all_wallet_keys = w.keys()
        >>> w.keys(depth=0) # doctest:+ELLIPSIS
        [<DbKey(id=..., name='ixogenlib_legacy_wallet_test', wif='xprv9s21ZrQH143K3cxbMVswDTYgAc9CeXABQjCD9zmXCpXw4MxN93LanEARbBmV3utHZS9Db4FX1C1RbC5KSNAjQ5WNJ1dDBJ34PjfiSgRvS8x'>]
    ...
    def utxos(self, account_id=None, network=None, min_confirms=0, key_id=None):
        ...
        [{'value': 100000000, 'script': '', 'output_n': 0, 'transaction_id': ..., 'spent': False, 'script_type': 'p2pkh', 'key_id': ..., 'address': '16QaHuFkfuebXGcYHmehRXBBX7RG9NbtLg', 'confirmations': 0, 'txid': '748799c9047321cb27a6320a827f1f69d767fe889c14bf11f27549638d566fe4', 'network_name': 'ixogen'}]
    ...
    def transactions(self, account_id=None, network=None, include_new=False, key_id=None, as_dict=False):
        ...
        >>> w = Wallet('ixogenlib_legacy_wallet_test')
        >>> w.transactions()
        [<WalletTransaction(input_count=0, output_count=1, status=confirmed, network=ixogen)>]
    ...
    def transaction_create(self, output_arr, input_arr=None, input_key_id=None, account_id=None, network=None, fee=None,
        ...
        >>> t
        <WalletTransaction(input_count=1, output_count=2, status=new, network=ixogen)>
        ...
        :param network: Network name. Leave empty for default network
        :type network: str
        :param fee: Set fee manually, leave empty to calculate fees automatically. Set fees in the smallest currency  denominator, for example satoshi's if you are using ixogens. You can also supply a string: 'low', 'normal' or 'high' to determine fees automatically.
        :type fee: int, str
        :param min_confirms: Minimal confirmation needed for an UTXO before it will be included in inputs. Default is 1 confirmation. Option is ignored if input_arr is provided.
        :type min_confirms: int
        :param max_utxos: Maximum number of UTXO's to use. Set to 1 for optimal privacy. Default is None: No maximum
    ...
    def send_to(self, to_address, amount, input_key_id=None, account_id=None, network=None, fee=None, min_confirms=1,
        ...
        :param amount: Output is the smallest denominator for this network (ie: Satoshi's for Ixogen), as Value object or value string as accepted by Value class
    ...
    def sweep(self, to_address, account_id=None, input_key_id=None, network=None, max_utxos=999, min_confirms=1,
        ...
        >>> t
        <WalletTransaction(input_count=1, output_count=1, status=new, network=ixogen)>
        >>> t.outputs # doctest:+ELLIPSIS
    ...
    def transaction_load(self, txid=None, filename=None):
        ...
        :param txid: Transaction ID. Transaction object will be read from .ixogenlib datadir
        :type txid: str
