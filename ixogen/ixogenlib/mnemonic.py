...
from ixogenlib.config.secp256k1 import secp256k1_n
...
class Mnemonic(object):
    ...
    def __init__(self, language=DEFAULT_LANGUAGE):
        ...
        """
        self._wordlist = []
        with Path(XCL_INSTALL_DIR, 'wordlist', '%s.txt' % language).open() as f:
    ...
    def detect_language(words):
        ...
        wlcount = {}
        for fn in Path(XCL_INSTALL_DIR, 'wordlist').iterdir():
            if fn.suffix == ".txt":
                with open(os.path.join(str(XCL_INSTALL_DIR), 'wordlist', fn), encoding='utf-8') as f:
