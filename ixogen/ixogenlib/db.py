...
from ixogenlib.main import *
from ixogenlib.encoding import aes_encrypt, aes_decrypt
...
class Db:
    """
    Ixogenlib Database object used by Service() and HDWallet() class. Initialize database and open session when
    ...
    def __init__(self, db_uri=None, password=None):
        ...
        if version_db[:3] == '0.4' and IXOGENLIB_VERSION[:3] >= '0.5':
        ...
        try:
            if IXOGENLIB_VERSION != version_db:
                _logger.warning("IxogenLib database (%s) is from different version then library code (%s). "
                                "Let's try to update database." % (version_db, IXOGENLIB_VERSION))
                db_update(self, version_db, IXOGENLIB_VERSION)
    ...
    def _import_config_data(ses):
        ...
        if not installation_date:
            session.merge(DbConfig(variable='version', value=IXOGENLIB_VERSION))
...
class DbConfig(Base):
    """
    IxogenLib configuration variables
...
class DbWallet(Base):
    ...
    network_name = Column(String(20), ForeignKey('networks.name'), doc="Name of network, i.e.: ixogen, litecoin")
...
class DbKey(Base):
    ...
    network_name = Column(String(20), ForeignKey('networks.name'),
                          doc="Name of key network, i.e. ixogen, litecoin, dash")
...
class DbNetwork(Base):
    ...
    Use the ixogenlib/data/networks.json file to view and manage settings.

    """
    __tablename__ = 'networks'
    name = Column(String(20), unique=True, primary_key=True, doc="Network name, i.e.: ixogen, litecoin, dash")
...
def db_update_version_id(db, version):
    _logger.info("Updated IxogenLib database to version %s" % version)
...
def db_update(db, version_db, code_version=IXOGENLIB_VERSION):
