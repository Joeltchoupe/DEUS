...
from ixogenlib.config.config import *


# Initialize logging
logger = logging.getLogger('ixogenlib')
logger.setLevel(LOGLEVEL)

if ENABLE_IXOGENLIB_LOGGING:
    handler = RotatingFileHandler(str(XCL_LOG_FILE), maxBytes=100 * 1024 * 1024, backupCount=2)
    ...
    logger.info('WELCOME TO IXOGENLIB - CRYPTOCURRENCY LIBRARY')
    logger.info('Version: %s' % IXOGENLIB_VERSION)
    logger.info('Logger name: %s' % logging.__name__)
    logger.info('Read config from: %s' % XCL_CONFIG_FILE)
    logger.info('Directory databases: %s' % XCL_DATABASE_DIR)
    logger.info('Default database: %s' % DEFAULT_DATABASE)
    logger.info('Logging to: %s' % XCL_LOG_FILE)
    logger.info('Directory for data files: %s' % XCL_DATA_DIR)
